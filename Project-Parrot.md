...
> [!IMPORTANT]  
> To add columns to the data tab after configuring please use the <code>Add a new column</code> button within the Setup dashboard. Adding new columns while the interface is being used by annotators may cause issues when writing to the sheet. The best course of action is to inform all annotators to pause using the interface, once the changes have been made and the new configuration has been submitted all annotators using the interface should refresh their instances. The configuration dashboard does **not** currently support deletion of columns. If you have accidentally created and configured a column that you do not wish to be displayed, simply set its column type to <code>Hidden</code> in the setup dashboard. Manual deletion (or insertion) of columns using the Google Sheets interface will break the interface.
...
| Checkbox     | Checkbox column types are used when a checkbox data validation is present in the corresponding column. Checkboxes are sorted by <code>Checkbox Groups</code>, allowing for the bundling of related data within the UI. **Every checkbox** must have an associated checkbox group. If there are no columns configured as <code>Checkbox Groups</code>, the checkbox option will be greyed-out until you create a checkbox group. |
| Checkbox Group     | A special column type which represents and labels a grouping of checkboxes. If you are looking to collect multi-select-like data, please create a checkbox group and configure checkboxes for each option.  |
